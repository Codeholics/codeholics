...
The system SHALL render a top app-bar containing (left) the `~/` home link, (center) primary navigation links for `Coding`, `SysAdmin`, `InfoSec`, `OS`, `Hardware`, and `Reviews`, and (right) utility controls (search, theme toggle). On wide viewports the primary navigation SHALL be visible inline; on narrow viewports the same destinations SHALL remain available through the mobile navigation panel.
